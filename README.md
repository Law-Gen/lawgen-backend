...
*   **Frictionless integration:** The details provided here are crucial for enabling different development teams to work in parallel and integrate their components effortlessly.
...
*   **Request Headers:**
    ...
    *   `Accept-Language: en` or `am` (optional, for explicit language preference).
...
*   **Authentication Mechanism:** JWT (JSON Web Tokens).
    *   Upon successful login/registration, the API Gateway (via User Service) issues an `access_token` (short-lived, ~15-60 min) and a `refresh_token` (longer-lived, ~7 days).
...
| `401 Unauthorized` | `AUTHENTICATION_FAILED` | "Invalid credentials."                     | Failed login, incorrect password.                                           |
...
| :----------------------- | :----- | :---------------------------------- | :--------------------------------------------------- | :------------------------------------------------------- | :----------------------------------------------------------- |
| `/auth/register`         | `POST` | Register a new user                 | `{"email": "user@example.com", "password": "securepassword", "language_preference": "en"}` | `201 Created` `{"message": "User registered successfully."}`           | `400 INVALID_INPUT`, `409 DUPLICATE_RESOURCE` (`email` already exists) |
| `/auth/login`            | `POST` | Authenticate user and get tokens    | `{"email": "user@example.com", "password": "securepassword"}` | `200 OK` `{"access_token": "jwt_token", "refresh_token": "refresh_token"}` | `401 AUTHENTICATION_FAILED`                                |
...
| `/users/me/password`     | `PUT`  | Change user's password              | `{"old_password": "old_pw", "new_password": "new_pw"}` | `200 OK` `{"message": "Password updated successfully."}`         | `400 INVALID_INPUT`, `401 AUTHENTICATION_FAILED` (old password incorrect) |
...
#### 4.4. Quizzes (Routed to Chat & Quiz Service - F)

| Endpoint               | Method | Description                         | Request Body (Example)                               | **Success Response (20x)**                                      | **Error Response (40x, 500)**                                  |
| :--------------------- | :----- | :---------------------------------- | :------------------------------------------------- | :-------------------------------------------------------------- | :------------------------------------------------------------- |
| `/quizzes`             | `GET`  | Get list of available quizzes       | *(Auth Header)*                                    | `200 OK` `[{"id": "quiz_id_1", "title": "Inheritance Law Basics"}, {"id": "quiz_id_2", "title": "Contract Law"}]` | `401 UNAUTHORIZED`                                             |
| `/quizzes/{quizId}`    | `GET`  | Get details of a specific quiz      | *(Auth Header)*                                    | `200 OK` `{"id": "quiz_id_1", "title": "...", "questions": [{"q_id": 1, "text": "...", "options": ["A", "B", "C"]}]}` | `401 UNAUTHORIZED`, `404 NOT_FOUND` (quizId invalid)           |
| `/quizzes/{quizId}/submit` | `POST` | Submit answers for a quiz           | `{"answers": [{"question_id": 1, "selected_option": "A"}]}` | `200 OK` `{"score": 80, "total_questions": 10, "results": [{"question_id": 1, "correct": true}]}` | `400 INVALID_INPUT`, `401 UNAUTHORIZED`, `404 NOT_FOUND` (quizId invalid) |
...
| `/content/search`       | `GET`  | Search legal content (query parameters) | `?query=property&category=civil`     | `200 OK` `[{"id": "doc_1", "title": "Article 842", "snippet": "..."}, {...}]` | `400 INVALID_INPUT` (invalid query/category)        |
| `/content/{contentId}`  | `GET`  | Get specific legal content              | *(Auth Header optional)*             | `200 OK` `{"id": "doc_1", "title": "Article 842", "full_text": "...", "source_url": "..."}` | `404 NOT_FOUND` (contentId invalid)                 |
...
#### 4.7. Admin Endpoints (Routed to Admin Service - I)
...
| `/admin/content`           | `POST` | Add new legal content             | `{"title": "New Proclamation", "text": "...", "category_id": "cat_1", "language": "am"}` | `201 Created` `{"message": "Content added.", "id": "new_id"}`   | `400 INVALID_INPUT`, `401 UNAUTHORIZED`, `403 ACCESS_DENIED` |
| `/admin/content/{contentId}` | `PUT`  | Update legal content              | `{"text": "Updated content text."}`                   | `200 OK` `{"message": "Content updated."}`                | `400 INVALID_INPUT`, `401 UNAUTHORIZED`, `403 ACCESS_DENIED`, `404 NOT_FOUND` |
...
| `/internal/quiz/content/{quizId}` | `F -> E`     | Retrieve quiz questions and correct answers     | *(Path parameter `quizId`)*                          | `200 OK` `{"id": "quiz_id", "title": "...", "questions": [{"q_id": 1, "text": "...", "options": [...]}, {...}], "correct_answers": [{"q_id": 1, "answer": "A"}]}` | `404 NOT_FOUND` (quizId invalid)                                       |
...
| `feedback.submitted`     | `Analytics & Feedback Service (H)` | *(None, H stores it)*        | `{"user_id": "uuid", "rating": 5, "comment": "...", "timestamp": "..."}` | Stores user feedback (H is the ultimate consumer). |
| `admin.content.updated`  | `Admin Service (I)`  | `AI Query & Content Service (E)` | `{"content_id": "doc_id", "action": "create"|"update"|"delete", "timestamp": "..."}` | Notifies the content service of updates to legal texts, potentially triggering cache invalidation or re-indexing. |
...
*   **User Service (C_DB):** `Users` (id, email, password_hash, language_preference, role, created_at, updated_at).
...
*   **AI Query & Content Service (E_DB):** `LegalContent` (id, title, category_id, full_text, language, source_url, version, last_updated), `Categories` (id, name).
*   **Chat & Quiz Service (F_DB):** `ChatSessions` (id, user_id, created_at), `ChatMessages` (id, session_id, sender, text, sources, is_disclaimer_included, timestamp), `Quizzes` (id, title, description, category), `Questions` (id, quiz_id, text, options, correct_answer), `UserQuizAttempts` (id, user_id, quiz_id, score, completed_at, answers).
